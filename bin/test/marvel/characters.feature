...
Scenario: Obtener personaje por ID (exitoso)
  ...
  * def characterRequest = call createUniqueCharacter
...
Scenario: Actualizar personaje (exitoso)
  # Primero creamos un personaje
  * def characterRequest = call createUniqueCharacter
  * def characterUpdateRequest = { name: '#(characterRequest.name)', alterego: '#(characterRequest.alterego)', description: 'Updated description', powers: '#(characterRequest.powers)' }
